...
my $caption2 = "\\textbf{List of VADR and FLAN influenza B, C, and D model reference sequences and attributes of associated proteins.} The ``\\#coords'' column indicates the number of distinct pairs of start and stop genome nucleotide coordinates for all proteins in the set. The ``\\# proteins'' column indicates the number of proteins in the set. Bold model accessions indicate models without an analog in FLAN. Italicized model accessions indicate models for which VADR uses a different accession than FLAN.";
