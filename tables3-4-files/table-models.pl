...
my $caption1 = "\\textbf{List of VADR and FLAN influenza A model reference sequences and attributes of associated proteins.} The ``\\#coords\'' column indicates the number of distinct pairs of start and stop reference genome nucleotide coordinates for all proteins in the set. The ``\\# proteins'' column indicates the number of proteins in the set, and the ``\\#extra'' column indicates the number of additional proteins added to the VADR protein set not present in the FLAN set based on analysis of training set results. For CDS that have italicized ``CDS product'' and ``gene'' names, FLAN errors are converted to warnings and VADR converts them to ``misc\\_feature'' features if they have certain usually fatal alerts, instead of failing the sequence. Bold model accessions indicate models without an analog in FLAN.";
